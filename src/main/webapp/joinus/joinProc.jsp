...
//에러체크 id가 null인지, 패스워드가 일치하지않는지
List<String> errors=new ArrayList();
if(id==null || id.equals(""))//아이디 입력상태
	errors.add("아이디없음");
if(!pwd.equals(pwd2))//패스워드불일치	
	errors.add("패스워드불일치");

if(errors.size()>0){//에러가 있다면
	//errors를 가지고(request에 담아서) join.jsp로
	request.setAttribute("errors", errors);
	request.getRequestDispatcher("join.jsp").forward(request, response);//애러를가지고 join으로 돌아가라는뜻
}else{
	// 회원 가입후 index.jsp로 돌아가기
